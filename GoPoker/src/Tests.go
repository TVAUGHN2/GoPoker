...
func handValueTestsPass() bool {
	...
	if noPair.Value != HighCard { fmt.Printf("ERROR: High Card != %s\n", noPair.Face); testsPass = false }
	if pair.Value != Pair { fmt.Printf("ERROR: Pair = %s\n", pair.Face); testsPass = false }
	if twoPairs.Value != TwoPairs { fmt.Printf("ERROR: Two Pair = %s\n", twoPairs.Face); testsPass = false }
	if set.Value != Set { fmt.Printf("ERROR: Three of a Kind = %s\n", set.Face); testsPass = false }
	if quads.Value != Quads { fmt.Printf("ERROR: Four of a Kind = %s\n", quads.Face); testsPass = false }
	if fullHouse.Value != FullHouse { fmt.Printf("ERROR: Full House = %s\n", fullHouse.Face); testsPass = false }
	if straight.Value != Straight { fmt.Printf("ERROR: Straight = %s\n", straight.Face); testsPass = false }
	if flush.Value != Flush { fmt.Printf("ERROR: Flush = %s\n", flush.Face); testsPass = false }
	if straightFlush.Value != StraightFlush { fmt.Printf("ERROR: Straight Flush = %s\n", straightFlush.Face); testsPass = false }
	if royalFlush.Value != RoyalFlush { fmt.Printf("ERROR: Royal Flush = %s\n", royalFlush.Face); testsPass = false }
	if aceLowStraight.Value != Straight { fmt.Printf("ERROR: Ace Low Straight = %s\n", aceLowStraight.Face); testsPass = false }
	if tooManyCards != nil { fmt.Printf("ERROR: Too Many Cards ==  %t\n", tooManyCards == nil); testsPass = false }
	if sameCards != nil { fmt.Printf("ERROR: Same Cards == %t\n", sameCards == nil); testsPass = false }
